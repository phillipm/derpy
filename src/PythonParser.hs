...
funcdef = ter "def" <~> ter "id" <~> parameters <~> ter ":" <~> suite ==>
            ...
            (catWSpace ("(def (" ++ fname) params) ++ ") (" ++ rest ++ "))")
...
trailer = p
  where
    p =     ter "(" <~> (eps "" <|> arglist) <~> ter ")"
              ==> (\(_,(args,_)) -> (catWSpace "(called" args) ++ ")")
...
trailerRep = p
  where p = eps "" <|> trailer <~> p ==> (\(s1,s2) -> catWSpace s1 s2)

testlist :: Parser String
testlist = test <~> testComRep <~> (eps "" <|> ter ",") ==> (\(s1,(s2,_))-> catWSpace s1 s2)
